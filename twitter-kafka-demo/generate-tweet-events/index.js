...
var averageDelay = 7500;  // in miliseconds
...
//var jeeconfSessions = JSON.parse(fs.readFileSync('devoxxUK2018.json', 'utf8'));
var jeeconfSessions = JSON.parse(fs.readFileSync('jeeconf2018.json', 'utf8'));
...
function produceTweetMessage(param) {
    ...
    if (coin < 0.6) // jeeconf
    {
        var sessionSeq = Math.floor((Math.random() * jeeconfSessions.length));
        var session = jeeconfSessions[sessionSeq];

        tweetEvent.tagFilter = "JEEConf"
        tweetEvent.text = `${prefix} at JEEConf 2018 ${session.title} by ${session.speakers}`
        tweetEvent.hashTag ='JavaDev@Kyiv'
    }
